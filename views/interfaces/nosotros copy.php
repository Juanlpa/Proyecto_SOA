...
$(document).ready(function() {
    ...
    $('#crudForm').submit(function(e) {
        ...
        var url = cedulaOriginal ? 'models/editar.php' : 'models/guardar.php';
        var data = {
            cedula: $('#cedula').val(),
            nombre: $('#nombre').val(),
        ...
        if (cedulaOriginal) {
            data.cedulaOriginal = cedulaOriginal;
        }
        $.ajax({
            url: url,
            method: 'POST',
    ...
    $('#tableBody').on('click', '.edit-btn', function() {
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function(data) {
                var estudiante = data.find(function(est) {
                    return est.CED_EST == cedula;
                });
                if (estudiante) {
                    $('#cedulaOriginal').val(estudiante.CED_EST);
                    $('#cedula').val(estudiante.CED_EST);
                    $('#nombre').val(estudiante.NOM_EST);
                    $('#apellido').val(estudiante.APE_EST);
                    $('#direccion').val(estudiante.DIR_EST);
                    $('#telefono').val(estudiante.TEL_EST);
                    $('#curID').val(estudiante.ID_CUR);
